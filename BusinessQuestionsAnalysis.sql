...
-- Q11. Which gender contributes more to total revenue, and how does their 
--revenue share compare to their share of the total customer base?
...
final AS (
    SELECT
        ...
        g.customer_count,
        g.total_revenue,
        ROUND(g.customer_count * 100.0 / o.total_customers, 2) AS customer_share_pct,
        ROUND(g.total_revenue * 100.0 / 
             (SELECT SUM(total_revenue) FROM gender_stats), 2) AS revenue_share_pct
    FROM gender_stats g
    CROSS JOIN overall o
)

SELECT *,
       ROUND(revenue_share_pct - customer_share_pct, 2) AS contribution_vs_population_pct
FROM final;

-- If you want to verify actual min/max ages inside each band
SELECT 
    age_group_fixed,
    MIN(age) AS min_age,
    MAX(age) AS max_age
FROM customer
GROUP BY age_group_fixed
order by min_age;

-- Q12. Which purchase frequency groups contribute the most in terms of customer volume 
-- and average spend?
SELECT 
